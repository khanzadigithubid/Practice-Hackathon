...
const Card: React.FC<CardProps> = ({ arr }) => {
  return (
    <>
      {arr.map((item) => (
        <Link href={`/productdetails/${item._id}`} key={item._id || item.title} className="sm:w-60 xs:w-52 w-36">
          ...
          <span className="font-medium mt-2 xs:text-base text-sm">{item.title}</span>
          <br />
